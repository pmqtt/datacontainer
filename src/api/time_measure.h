...
template<std::size_t loop_count = 1,typename TimeT = std::chrono::milliseconds>
struct measure
{
	measure(){
	}
		...
		decltype(auto) INVOKE(const std::string & name, Type T::* f, T1&& t1, Args&&... args)
	{
        ...
        exec_map[name] = (double)(exec_duration / loop_count);
	}
...
private:
	std::map<std::string,double> exec_map;
};
