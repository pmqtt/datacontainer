...
class time_tree{
    struct iterator
    {
        ...
        iterator(std::vector<time_tree_node<VALUE>> * tree, std::size_t parent_index, std::size_t child_index ) : inner_tree(tree){
...
public:
    ...
    void add(const time_tree_item<VALUE> & item){
        if(tree.empty()){
            ...
            node.end_point = node.start_point + std::chrono::duration_cast<std::chrono::milliseconds>(interval_width);
            ...
            if ( tree[start_index].items.empty() ){
                std::chrono::milliseconds startPointMS = tree[0].start_point + (std::chrono::duration_cast<std::chrono::milliseconds>(interval_width) * start_index);
                tree[start_index].start_point = startPointMS;
                tree[start_index].end_point = startPointMS + std::chrono::duration_cast<std::chrono::milliseconds>(interval_width);
    ...
    std::vector<time_tree_item<VALUE>> find_in_range(std::chrono::milliseconds left, std::chrono::milliseconds right){
    ...
    std::pair<tree_index_point,tree_index_point> get_interval_range(std::chrono::milliseconds left, std::chrono::milliseconds reight){
        return {
                get_left_point(left),
...
private:
    ...
    INDEX_T binsearch(const std::vector<time_tree_item<VALUE>> & items, INDEX_T start, INDEX_T end, std::chrono::milliseconds timepoint){
